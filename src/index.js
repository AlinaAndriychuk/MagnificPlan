...
function PopupBlock(props) {
  ...
  let colorOfTask = "#fbfeffde";

  function showPopup() {
    setFloated(false);
  };

  function changeColorOfTask(color) {
    taskName.current.style.backgroundColor = color;
    colorOfTask = color;
  };

  function hidePopup() {
    props.saveFunction(props.blockId, taskName.current.innerText, props.taskIndex, description.current.value, hours.current.value, minutes.current.value, files.current.innerHTML, colorOfTask);
    setFloated(true);
  };
    ...
    return (
      <div className='popup-wrapper'>
        <div className="popup">
          ...
          <Colorpalette colorFunction={changeColorOfTask}></Colorpalette>
...
function Colorpalette(props) {
  ...
  function changeBackground(index) {
    props.colorFunction(arrayOfColors[index]);
    setColored(true);
  };
      ...
      <button className="planner__palette-button">
        {
          colorIndexes.map ((item) => {
            ...
            }} ref={color} style={styleForColor[item]} className='planner__palette' key = {item} index={item}></div>)
          })
        }
      </button>
    );
...
function MenuBar(props) {
  ...
  function hidePopup() {
    document.getElementsByClassName("planner-bar-small")[0].style.paddingLeft = "0";
    setShowBar(false);
  };
...
function ChooseBar() { 
  ...
  function updateTextInBlock( id, text, index, desc, hours, minutes, files, color) {
    let fullBlockField = boardDetails.blockField;
    let newBlockField = boardDetails.blockField[id];

    newBlockField.tasks[index] = text;
    if (color === "" || color) newBlockField.colors[index] = color;
            ...
            <div key= {id} className="popup-wrapper desk-popup">
              <div className="popup">
                ...
                <button className="planner__palette-button">
                {
                  colorIndexes.map ((item) => {
                    return (
                      <div onClick={()=> changeBackground(item)} ref={color} style={styleForColor[item]} className='planner__palette' key = {item} index={item}></div>
                    )
                  })
                }
                </button>
...
if (document.documentElement.clientWidth >= 845) {
  gsap.from(".header__item:first-child", {duration: 2, y: 100, opacity: 0});
  gsap.from(".header__item:last-child", {duration: 2, delay: 1, y: 100, opacity: 0});
} else {
  gsap.from(".header__item:first-child", {duration: 2, y: 50});
  gsap.from(".header__item:last-child", {duration: 2, y: -50});
};

window.addEventListener("scroll", function() {
  if (document.documentElement.clientWidth >= 845) {
    if (document.documentElement.scrollTop > 600 && document.documentElement.scrollTop < 1250) {
    ...
    if (document.documentElement.scrollTop > 1950 && document.documentElement.scrollTop < 2700) {
  ...
  if (document.documentElement.clientWidth < 845 && document.documentElement.clientWidth >= 384 ) {
    ...
    if (document.documentElement.scrollTop > 2200 && document.documentElement.scrollTop < 2750) {
      gsap.to(".quote-block__title", {duration: 2, opacity: 1, rotateX: 0});
    };
